...
describe('User Controller test', () => {
	...
	it('createToken should generate a valid token with the given ID', () => {
		...
		expect(jwt.sign).toHaveBeenCalledWith({ _id: '12345' }, 'my-secret-key', { expiresIn: '1h' });
	});
	...
	it('respond to creating new user with status 200', async () => {
		...
		const res = await request(api).post('/users/signup').send(testData).set('Content-Type', 'application/json').expect(200);
